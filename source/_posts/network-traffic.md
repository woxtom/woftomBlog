...
categories: [技术, 折腾日志]
cover: https://images.unsplash.com/photo-1544197150-b99a580bb7a8?q=80&w=2000&auto=format&fit=crop
---

作为一个计算机专业的学生，最近在搞科研查文献和访问校内资源时遇到个痛点：开着代理（Clash）访问校内网看课程回放时速度很慢（v.sjtu.edu.cn），关了代理又上不去GitHub用不了chatgpt。

难道我还要频繁开关 VPN 吗？难道我要一次使用一个吗？显然不。不仅如此，我还解决了一下广告拦截和隐私保护的问题，顺便预习了一波计算机网络原理。

<!--more-->

## 0x01 代理与局域网的冲突

起因很简单，当我在使用 Clash Verge 访问外网查资料时，突然需要访问学校的 `v.sjtu.edu.cn` 观看课程回放。结果浏览器一直转圈。

### 原理分析
**Clash 本质上是一个基于规则的流量转发器**。
当开启“全局模式”或者默认规则配置不当时，Clash 这个“传达室大爷”会把发往 `sjtu.edu.cn` 的请求也打包发给了位于美国的代理服务器。
美国服务器拿着请求去访问交大内网，不仅绕了地球一圈延迟极高，而且大概率会被学校防火墙拦截。
...
3.  **生产力工具的流量**：
    `api.individual.githubcopilot.com` 一直在活跃。作为 CS 学生，Copilot 实时把我的代码上下文发回 GitHub 进行推理，这也解释了为什么使用copilot补全时必须挂代理，否则补全会卡顿。

## 0x04 踩坑记录：30GB 日志

在折腾过程中，我也犯了个新手错误。我之前开启了 `log-level: debug` 并且配置了日志写入文件。结果没过多久，我发现 C 盘红了——Clash 生成了一个 **30GB** 的日志文件！

**教训：**
1.  日常使用务必将日志等级设为 `Info` 或 `Silent`。
2.  除非调试，否则不要配置 `log-file`。
3.  如果界面显示 Empty Log，记得**重启内核**来重载配置。
...
**工具的意义，在于让我们更高效地探索世界，同时保护我们不被数字洪流淹没。**
